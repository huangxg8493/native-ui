...
## 修改密码功能独立化设计决策
- **背景**: 当前编辑弹窗中包含密码字段，用于修改密码。为明确职责，将密码修改功能独立出来
- **编辑按钮**: 仅编辑用户基本信息（用户名、邮箱、省市区、状态），不再包含密码修改
- **修改密码按钮**: 独立的修改密码流程
- **操作列按钮**: `编辑` | `修改密码` | `删除` | `分配角色`（共4个按钮）
- **编辑弹窗调整**: 移除密码字段，保留手机号（只读）、用户名、邮箱、省市区、状态、创建/更新时间（只读）
- **修改密码弹窗字段**: 原密码、修改密码、确认密码
- **修改密码接口**: `POST /api/users/update/pwd`，参数 userId、phone、password（原密码）、newPassword（新密码）
- **修改密码校验**: 新密码最少6位，确认密码必须与新密码一致
- **修改密码成功**: 提示"密码修改成功，即将跳转到登录页"，2秒后自动跳转 login.html
- **涉及文件**: `html/sys/user.html`（移除密码行、新增修改密码弹窗）、`assets/script/user.js`（移除密码逻辑、新增修改密码逻辑）、`assets/css/user.css`（新增 changePwdBtn 样式）
